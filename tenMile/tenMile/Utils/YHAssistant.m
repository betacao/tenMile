...
//    CGFloat quality = 1.0f;
    if (compress) {
        NSString *photoQuality = [[NSUserDefaults standardUserDefaults] objectForKey:kStoreKeyPhotoQuality];
...
- (void)alertNetwork
{
    ...
    [self alert:@"网络数据获取失败,请重试！" type:YHAlertTypeNetwork];
}
...
+ (id)buttonWithImageName:(NSString *)imageName highlightedImageName:(NSString *)highlightedImageName title:(NSString *)title target:(id)target action:(SEL)action
{
    ...
    [button setFrame: CGRectMake(0.0f, 0.0f, imageSize.width, imageSize.height)];
    if (title != nil) {
...
//    frame.origin.y = -7.0f;
//    button.frame = frame;
    UIView *menuButtonContainer = [[UIView alloc] initWithFrame:button.bounds];
...
- (void)removeFileAtPath:(NSString *)path fileExtesion:(NSArray *)extesions
{
    NSArray *files = [self contentsOfDirectoryAtPath:path error:nil];
    for (NSString *filePath in files) {
        if(extesions == nil || [extesions count] == 0 || [extesions containsObject:[filePath pathExtension]]){
            NSString *fullPath = [path stringByAppendingPathComponent:filePath];
            [self removeItemAtPath:fullPath error:nil];
        }
    }
